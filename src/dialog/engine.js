...
class DialogEngine {
  constructor({ flowsProvider, stateManager, notifications, options, logger = loggerShim }) {
    Object.assign(this, { logger, flowsProvider, notifications, stateManager })
    ...
    flowsProvider.on('flowsChanged', () => Object.assign(this, { flows: [], flowsLoaded: false }))
  }
  ...
  async processMessage(stateId, event) {
    try {
      ...
      this.notifications.create({ message: e.message, level: 'error', redirectUrl: '/logs' })
    }
  }
